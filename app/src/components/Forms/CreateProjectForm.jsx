...
const CreateProjectForm = () => {
	...
	const submitProject = async e => {
		...
		try {
			await newReq.createProject({ name: project.name, description: project.description, label: project.label })
		...
		<>
			...
			<div>
				<form className='flex flex-col text-white p-8 bg-[#393552] rounded-sm' onSubmit={submitProject}>
					...
					<input onChange={handleChange} className='bg-transparent border-b-2 p-4' placeholder='Project name' name='name' type={'text'} />
					<textarea onChange={handleChange} className='bg-transparent border-b-2 p-4' placeholder='Write a description' name='description'></textarea>
					<label className='my-6 flex justify-between'>
						...
						<input onChange={handleChange} className='bg-transparent border-b-2 w-72' placeholder='Write a label' />
					</label>
