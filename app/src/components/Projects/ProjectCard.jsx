...
const ProjectCard = (props) => {
	...
	return (
		<div onMouseLeave={handleStop} onMouseMove={handleMovemenet} id={`wrapper-${props.id}`} className='bg-gradient-to-r from-[#26233a] to-[#29273d] max-w-2xl w-full rounded-lg [perspective:1000px] shadow-2xl opacity-1'>
			<div  className='p-4 flex justify-between w-full'>
				<span className='text-3xl font-extrabold text-[#ebbcba]'>{props.name}</span>
				<span className='text-lg text-[#31748f]'>{props.createdAt.split('T')[0]}</span>
			</div>
			<div className='min-h-[300px] flex justify-between'>
				<p className='m-4 text-white text-xl'>{props.description}</p>
